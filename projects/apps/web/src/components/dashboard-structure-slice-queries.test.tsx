...
import type { DashboardStructureDeployFlow } from './dashboard-structure-runtime-context.js';
...
function CompareQueryState({ guildId }: { guildId: string }) {
    useDashboardStructureBackupsQuery(guildId);
    useDashboardStructureRunsQuery(guildId);
    return null;
}

function BackupsQueryState({ guildId }: { guildId: string }) {
    const query = useDashboardStructureBackupsQuery(guildId);
    return <p>{`${guildId}:${query.status}`}</p>;
}

function RunsQueryState({ guildId }: { guildId: string }) {
    const query = useDashboardStructureRunsQuery(guildId);
    return <p>{`${guildId}:${query.status}`}</p>;
}
        ...
        <QueryClientProvider client={queryClient}>
            <DashboardStructureRuntimeProvider
                value={{
                    ...
                    retryStatus: vi.fn(),
                    executionProgress: { retry: vi.fn(), retrying: false, transport: { mode: 'idle' } },
                }}>
                {children}
            </DashboardStructureRuntimeProvider>
        </QueryClientProvider>
