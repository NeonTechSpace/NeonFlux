...
import { DashboardBlueprintBackupHistory } from './dashboard-blueprint-backup-history.js';

const backup: DashboardBlueprintBackupSummary = {
    id: 'backup-1',
...
describe('DashboardBlueprintBackupHistory', () => {
    ...
    it('preserves paged loading and submits restore-point deletion to the authoritative policy', () => {
        const view = renderHistory({
            page: {
        ...
        expect(view.onDelete).toHaveBeenCalledWith(expect.objectContaining({ source: 'restore_point' }));
        expect(view.onLoadMore).toHaveBeenCalledOnce();
    });
});

function renderHistory(
    overrides: Partial<Parameters<typeof DashboardBlueprintBackupHistory>[0]> = {}
): ReturnType<typeof createHandlers> {
    const handlers = createHandlers();

    render(
        <DashboardBlueprintBackupHistory
            page={{ backups: [backup] }}
