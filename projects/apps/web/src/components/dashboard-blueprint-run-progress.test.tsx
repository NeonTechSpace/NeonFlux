...
describe('Server Blueprint progress transport', () => {
    ...
    it('stays idle when no deployment run is active', () => {
        renderProgress({ initialRun: undefined, planId: undefined });
    ...
    it('uses the authenticated polling read when the live watch fails', async () => {
        const poll = deferred<RunQueryResult>();
        ...
        liveClient.callback?.();
        poll.resolve(runQueryResult({ appliedSteps: 1, updatedAt: '2026-07-11T12:00:01.000Z' }));
    ...
    it('keeps live progress healthy when the polling read fails', async () => {
        ...
        await waitFor(() => expect(screen.getByTestId('progress').textContent).toBe('run-1:running:1/2'));
        await waitFor(() =>
            expect(queryClient.getQueryState(blueprintProgressKey('guild-1', 'run-1'))?.status).toBe('error')
        );
        expect(screen.getByTestId('issue').textContent).toBe('none');
    ...
    it('exposes pending state while an explicit polling retry is unresolved', async () => {
        const retryPoll = deferred<RunQueryResult>();
        mocks.httpQuery.mockResolvedValueOnce(
            runQueryResult({ appliedSteps: 1, updatedAt: '2026-07-11T12:00:01.000Z' })
        );
        renderProgress({ initialRun: run(), planId: 'run-1' });
        ...
        retryPoll.resolve(runQueryResult({ appliedSteps: 1, updatedAt: '2026-07-11T12:00:01.000Z' }));
        ...
        await waitFor(() =>
            expect(
                invalidate.mock.calls.filter(
                    ([filters]) => JSON.stringify(filters?.queryKey) === JSON.stringify(blueprintSettingsKey('guild-1'))
                )
            ).toHaveLength(1)
        );
    ...
    it('does not let updates from the previous run replace the selected run', async () => {
        mocks.httpQuery.mockImplementation((_query, args: { planId: string }) =>
            Promise.resolve(
                runQueryResult({
                    id: args.planId,
                    totalSteps: args.planId === 'run-1' ? 2 : 4,
        ...
        view.rerender(
            <ProgressProvider queryClient={view.queryClient}>
                <ProgressProbe initialRun={run({ id: 'run-2', totalSteps: 4 })} planId='run-2' />
            </ProgressProvider>
        );
    ...
    it('identifies a missing progress function as backend incompatibility without polling retries', async () => {
        const missingFunction = new Error(
            "Could not find public function for 'blueprint:findBlueprintRunProgressForGuild'"
        );
    ...
    it.each([
        ...
        ['explicit protocol version mismatch', new Error('Run protocolVersion mismatch: expected 2, received 1')],
    ...
    it('distinguishes an incompatible durable run row from backend deployment skew', async () => {
        const incompatible = runQueryResult({
        ...
        await waitFor(() =>
            expect(screen.getByTestId('issue').textContent).toBe('BLUEPRINT_RUN_PROTOCOL_INCOMPATIBLE')
        );
        expect(screen.getByTestId('progress').textContent).toBe('run-1:running:0/2');
        expect(mocks.httpQuery).toHaveBeenCalledOnce();
    });
    ...
    it('does not misclassify an unrelated argument validation failure as deployment skew', async () => {
        const invalidPlanId = new Error(
            'ArgumentValidationError: Value does not match validator. Path: .planId Value: "invalid" Validator: v.id("blueprintPlans")'
        );
        mocks.httpQuery.mockRejectedValue(invalidPlanId);
        renderProgress({ initialRun: run(), planId: 'run-1' });
...
function ProgressProbe({
    ...
    const progress = useDashboardBlueprintRunProgress({
        guildId: 'guild-1',
        initialRun,
        planId,
    });
    return (
        <>
            <output data-testid='progress'>
                {progress.run
                    ? `${progress.run.id}:${progress.run.status}:${progress.run.completedSteps}/${progress.run.totalSteps}`
                    : 'none'}
            </output>
...
function renderProgress({
    ...
    const view = render(
        <ProgressProvider queryClient={queryClient}>
            <ProgressProbe initialRun={initialRun} planId={planId} />
        </ProgressProvider>
    );
...
function run(overrides: Partial<DashboardBlueprintRunProgress> = {}): DashboardBlueprintRunProgress {
    return {
        id: 'run-1',
...
function runQueryResult(overrides: Partial<RunQueryResult> = {}): RunQueryResult {
    return {
        appliedSteps: 0,
        createdAt: '2026-07-11T12:00:00.000Z',
        failedSteps: 0,
...
function blueprintSettingsKey(guildId: string) {
    return ['dashboard', 'guild', guildId, 'blueprint', 'runs'];
}

function blueprintProgressKey(guildId: string, planId: string) {
    return ['dashboard', 'guild', guildId, 'blueprint-run-progress', planId];
}
