...
import { formatDashboardBlueprintChannelType } from './dashboard-blueprint-explorer-channel-types.js';

export function DashboardBlueprintExplorerDetails({
    metadata,
    source,
}: {
    metadata: DashboardBlueprintExplorerPathMetadata | undefined;
        ...
        <aside className='min-w-0 p-4' aria-label='Blueprint inspector'>
            ...
            <dl className='mt-1 divide-y divide-[var(--dash-border)]'>
                ...
                {metadata.kind === 'channel' || metadata.kind === 'category' ? (
                    <DetailValue
                        label='Channel type'
                        value={formatDashboardBlueprintChannelType(readItemValue(metadata.item, 'type'))}
                    />
                ) : null}
            ...
            {metadata.actions.length > 0 ? (
                <section className='mt-4 border-t border-[var(--dash-border)] pt-3'>
                    <p className='text-xs font-medium text-[var(--dash-text-subtle)]'>Related plan steps</p>
...
function LinkUrlDetail({ item }: { item: DashboardBlueprintExplorerPathMetadata['item'] }) {
...
function ActionDetails({ action }: { action: DashboardBlueprintExplorerPlanStep }) {
...
function readActionChanges(action: DashboardBlueprintExplorerPlanStep): Array<{
    after: unknown;
    before: unknown;
    field: string;
}> {
...
function readPermissionOverwrites(
    item: DashboardBlueprintExplorerPathMetadata['item']
...
function formatPermissionTarget(id: string, type: number, source: DashboardBlueprintExplorerSource): string {
