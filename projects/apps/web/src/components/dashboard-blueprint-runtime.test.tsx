...
import { DashboardBlueprintRuntime } from './dashboard-blueprint-runtime.js';

const routerHarness = vi.hoisted(() => ({
    navigate: vi.fn(() => Promise.resolve()),
    state: {
        isLoading: false,
        location: { pathname: '/dashboard/guild-1/blueprint/current' },
        resolvedLocation: { pathname: '/dashboard/guild-1/blueprint/current' },
    },
}));
...
vi.mock('@tanstack/react-router', async () => {
    ...
    const { useDashboardBlueprintRuntime } = await import('./dashboard-blueprint-runtime-context.js');
...
vi.mock('./dashboard-blueprint-run-progress.js', () => ({
    useDashboardBlueprintRunProgress: () => ({
        run: undefined,
        issueCode: undefined,
        retry: vi.fn(),
        retrying: false,
...
describe('DashboardBlueprintRuntime', () => {
    beforeEach(() => {
    ...
    it('keeps its draft while history or programmatic navigation shows the target leaf island', () => {
        const view = render(<DashboardBlueprintRuntime guildId='guild-1' />);
        ...
        routerHarness.state = {
            isLoading: true,
            location: { pathname: '/dashboard/guild-1/blueprint/backups' },
            resolvedLocation: { pathname: '/dashboard/guild-1/blueprint/current' },
        };
        view.rerender(<DashboardBlueprintRuntime guildId='guild-1' />);
        ...
        routerHarness.state = {
            isLoading: false,
            location: { pathname: '/dashboard/guild-1/blueprint/backups' },
            resolvedLocation: { pathname: '/dashboard/guild-1/blueprint/backups' },
        };
        view.rerender(<DashboardBlueprintRuntime guildId='guild-1' />);
    ...
    it.each(['/dashboard/guild-1/blueprint', '/dashboard/guild-1/blueprint/'])(
