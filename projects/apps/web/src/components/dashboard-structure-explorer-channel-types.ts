...
export function buildDashboardStructureChannelTypeCss(explorerModel: DashboardStructureExplorerModel): string {
...
export function useDashboardStructureChannelTypeAccessibilityLabels(
    ...
    useEffect(() => {
        ...
        const updateLabels = () => {
            for (const row of treeRoot.querySelectorAll<HTMLElement>("button[data-type='item'][data-item-path]")) {
                ...
                const typeLabel = formatDashboardStructureChannelType(readChannelType(metadata.item));
...
export function formatDashboardStructureChannelType(value: unknown): string {
...
function readChannelType(item: DashboardStructureExplorerPathMetadata['item']): number | undefined {
