...
import {
    buildDashboardBlueprintChannelTypeCss,
    useDashboardBlueprintChannelTypeAccessibilityLabels,
} from './dashboard-blueprint-explorer-channel-types.js';
import { DashboardBlueprintExplorerDetails } from './dashboard-blueprint-explorer-details.js';
...
type DriftState = Extract<DashboardBlueprintDriftResult, { type: 'structure-drift' }>;

export function DashboardBlueprintExplorer({
    busyAction,
    drift,
    overlayMode,
    preflightByPlanId,
    ...
    overlayMode: DashboardBlueprintExplorerOverlayMode;
    preflightByPlanId: Record<string, DashboardBlueprintPreflightReport>;
    plans: DashboardBlueprintPlan[];
    section: DashboardBlueprintExplorerSection;
    selectedEntityKey: DashboardBlueprintExplorerEntityKey | undefined;
    comparisonTarget: DashboardBlueprintExplorerComparisonTarget;
    source: DashboardBlueprintExplorerSource;
    onCompareDriftBaseline: () => void;
    onCompareImportJson: () => void;
    onCompareLive: () => void;
    onCompareRequestedFinalState: (plan: DashboardBlueprintPlan) => void;
    onInspectImportJson: () => void;
    onInspectRequestedFinalState: (plan: DashboardBlueprintPlan) => void;
    onLoadPlanSteps: (plan: DashboardBlueprintPlan) => void;
    onLoadLive: () => void;
    onOverlayModeChange: (mode: DashboardBlueprintExplorerOverlayMode) => void;
    onSectionChange: (section: DashboardBlueprintExplorerSection) => void;
    onSelectedEntityKeyChange: (key: DashboardBlueprintExplorerEntityKey | undefined) => void;
    presentation?: 'default' | 'review';
}) {
    const [viewMode, setViewMode] = useState<'diff' | 'tree'>('tree');
    const selectedPlan = readSelectedPlan(overlayMode, plans);
    const overlayActions = useMemo(
        () => readOverlayActions({ drift, overlayMode, preflightByPlanId, selectedPlan }),
        [drift, overlayMode, preflightByPlanId, selectedPlan]
    );
    const explorerModel = useMemo(
        () =>
            buildDashboardBlueprintExplorerModel({
                ...
                preflightReport: selectedPlan ? preflightByPlanId[selectedPlan.id] : undefined,
                section,
                snapshot: source.snapshot,
            }),
        [drift?.hasMorePreview, overlayActions, overlayMode, preflightByPlanId, section, selectedPlan, source.snapshot]
    );
    ...
    const isPlanStepBusy = selectedPlan ? busyAction === `plan-steps:${selectedPlan.id}` : false;
    const canCompare = Boolean(source.canonicalJson);
    const canCompareDriftBaseline = drift?.baseline.status === 'succeeded';
    const canInspectRequestedFinalState = Boolean(selectedPlan?.requestedSnapshot);
    const jsonDiff = useMemo(
        () =>
            source.canonicalJson && comparisonTarget.canonicalJson
                ? buildDashboardBlueprintExplorerJsonDiff({
    ...
    return (
        <section
                    ...
                    <label className='min-w-[11rem] text-xs font-medium text-[var(--dash-text-muted)]'>
                        ...
                        <select
                            ...
                            onChange={(event) => {
                                ...
                                if (action === 'requested-final' && selectedPlan) {
                                    onInspectRequestedFinalState(selectedPlan);
                                }
                            }}
                    ...
                    <label className='min-w-[12rem] text-xs font-medium text-[var(--dash-text-muted)]'>
                        ...
                        <select
                            ...
                            onChange={(event) => {
                                ...
                                if (action === 'requested-final' && selectedPlan) {
                                    onCompareRequestedFinalState(selectedPlan);
                                }
                            }}
            ...
            {viewMode === 'diff' ? (
                <DashboardBlueprintExplorerJsonDiffView
                ...
                <div
                    ...
                    <div className='min-w-0 space-y-3 p-3'>
                        ...
                        {selectedPlan && selectedPlan.planStepCount > selectedPlan.steps.length ? (
                            <div className='flex flex-wrap items-center justify-between gap-2 rounded-[var(--dash-radius-control)] border border-[var(--dash-border)] bg-[var(--dash-bg)] p-2'>
                                <p className='text-xs leading-5 text-[var(--dash-text-muted)]'>
                                    Plan step list is not fully loaded.
                                </p>
                                <button
                                    type='button'
                                    onClick={() => onLoadPlanSteps(selectedPlan)}
                                    disabled={Boolean(busyAction)}
                                    className={`${dashboardQuietActionClassName} min-h-8 px-2 text-xs`}>
                                    {isPlanStepBusy ? 'Loading' : 'Load plan steps'}
                                </button>
                            </div>
                        ) : null}
                    ...
                    <div className='min-w-0 @min-[48rem]/blueprint-explorer:max-h-[min(34rem,65dvh)] @min-[48rem]/blueprint-explorer:overflow-y-auto'>
                        <DashboardBlueprintExplorerDetails metadata={selectedMetadata} source={source} />
                    </div>
                </div>
            )}
        </section>
...
function ExplorerSectionControl({
    ...
    counts: DashboardBlueprintExplorerModel['counts'];
    section: DashboardBlueprintExplorerSection;
    onSectionChange: (section: DashboardBlueprintExplorerSection) => void;
}) {
    return (
    ...
    return (
        <label className='flex min-w-0 items-center justify-between gap-3 border-b border-[var(--dash-border)] pb-3 text-xs font-medium text-[var(--dash-text-muted)]'>
            ...
            <select
                ...
                value={overlayMode}
                onChange={(event) =>
                    onOverlayModeChange(event.currentTarget.value as DashboardBlueprintExplorerOverlayMode)
                }
...
function StructureExplorerTreeInstance({
    ...
    const channelTypeCss = useMemo(() => buildDashboardBlueprintChannelTypeCss(explorerModel), [explorerModel]);
    ...
    useDashboardBlueprintChannelTypeAccessibilityLabels(treeContainerRef, explorerModel);
...
function readOverlayActions({
    ...
    overlayMode: DashboardBlueprintExplorerOverlayMode;
    preflightByPlanId: Record<string, DashboardBlueprintPreflightReport>;
    selectedPlan: DashboardBlueprintPlan | undefined;
}): DashboardBlueprintExplorerPlanStep[] {
    if (overlayMode === 'drift') return drift ? toDashboardBlueprintExplorerPlanSteps(drift.previewActions) : [];
    if (overlayMode === 'none' || !selectedPlan) return [];

    return toDashboardBlueprintExplorerPlanSteps(selectedPlan.steps, preflightByPlanId[selectedPlan.id]);
}

function readSelectedPlan(
    overlayMode: DashboardBlueprintExplorerOverlayMode,
    plans: DashboardBlueprintPlan[]
): DashboardBlueprintPlan | undefined {
    if (!overlayMode.startsWith('plan:')) return undefined;

    const planId = overlayMode.slice('plan:'.length);
