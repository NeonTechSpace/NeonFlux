...
export type DashboardBlueprintBackupHealthIssue = {
    detail: string;
    href: string;
    tone: 'error' | 'warning';
    title: string;
...
export function deriveDashboardBlueprintBackupHealth({
    backups,
    formatDate,
    nowMs = Date.now(),
    settings,
    ...
    nowMs?: number;
    settings: DashboardBlueprintBackupSettings;
}): DashboardBlueprintBackupHealthIssue[] {
    ...
    const failedBackupForLink =
        latestRegularBackup?.status === 'failed'
            ...
            : settings.lastErrorMessage
              ...
              : undefined;
    const issues: DashboardBlueprintBackupHealthIssue[] = [];
    ...
    if (settings.lastErrorMessage || latestRegularBackup?.status === 'failed') {
        ...
        issues.push({
            ...
            href: failedBackupForLink
                ? `#${buildDashboardBlueprintBackupRowId(failedBackupForLink.id)}`
                : backupSettingsHref,
...
export function buildDashboardBlueprintBackupRowId(backupId: string): string {
...
function isFailedBackup(backup: DashboardBlueprintBackupSummary): boolean {
