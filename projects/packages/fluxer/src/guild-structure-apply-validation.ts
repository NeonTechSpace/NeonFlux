...
export type StructureApplyActionForValidation = {
    id: string;
    actionType: string;
    targetType: string;
    targetId?: string;
...
export type StructureReferenceMapping = {
    guildId: string;
...
export type StructureTargetKind = 'role' | 'category' | 'channel';

export type StructureActionReferenceValidationResult =
    | { ok: true }
    | {
          ok: false;
          actionId: string;
...
export type StructureActionReferenceValidationFailure = Exclude<StructureActionReferenceValidationResult, { ok: true }>;

type StructureActionReferenceValidationError = Omit<StructureActionReferenceValidationFailure, 'actionId' | 'ok'>;

type StructureOrderNormalizationResult<T> =
    | { ok: true; positions: T }
    | { ok: false; errorType: 'structure-order-mapping-missing' | 'structure-order-plan-invalid' };

export type StructureActionAuthoritySnapshot = {
    idMap: Record<string, string>;
    knownTargetKinds: Record<string, StructureTargetKind>;
};

export type StructureActionAuthorityProgressionResult<TVisitorError> =
    | (StructureActionAuthoritySnapshot & {
          durableIdMap: Record<string, string>;
          ok: true;
          steps: StructureActionAuthoritySnapshot[];
      })
    | {
          actionId: string;
          failure:
              | { type: 'reference'; error: StructureActionReferenceValidationError }
              | { type: 'visitor'; error: TVisitorError };
          ok: false;
      };

export function createStructureReferenceMapping(input: {
    guildId: string;
    idMap?: Record<string, string>;
    knownTargetIds?: readonly string[];
    knownTargetKinds?: Readonly<Record<string, StructureTargetKind>>;
    sourceGuildId?: string;
}): StructureReferenceMapping {
    return {
...
export function resolveStructureReference(id: string, mapping: StructureReferenceMapping): string | undefined {
    return mapping.idMap[id] ?? (mapping.knownTargetIds.has(id) ? id : undefined);
}

export function validateFluxerBotGuildStructureActionReferences(input: {
    actions: readonly StructureApplyActionForValidation[];
    guildId: string;
    idMap?: Record<string, string>;
    knownTargetKinds: Readonly<Record<string, StructureTargetKind>>;
    sourceGuildId?: string;
}): StructureActionReferenceValidationResult {
    const progression = simulateFluxerBotGuildStructureActionAuthority(input);
...
export function simulateFluxerBotGuildStructureActionAuthority<TVisitorError = never>(input: {
    actions: readonly StructureApplyActionForValidation[];
    guildId: string;
    idMap?: Record<string, string>;
    knownTargetKinds: Readonly<Record<string, StructureTargetKind>>;
    resolveCreatedTargetId?: (sourceId: string, action: StructureApplyActionForValidation) => string | undefined;
    sourceGuildId?: string;
    visitAction?: (
        action: StructureApplyActionForValidation,
        authority: StructureActionAuthoritySnapshot
    ) => TVisitorError | undefined;
}): StructureActionAuthorityProgressionResult<TVisitorError> {
    const kinds = new Map<string, StructureTargetKind>();
    const available = new Set<string>();
    const failReference = (
        actionId: string,
        error: StructureActionReferenceValidationError
    ): StructureActionAuthorityProgressionResult<TVisitorError> => ({
        actionId,
    ...
    for (const [idValue, kind] of Object.entries(input.knownTargetKinds)) {
        ...
        if (!id || id !== idValue || !isStructureTargetKind(kind) || kinds.has(id)) {
            return failReference(input.actions[0]?.id ?? 'execution-plan', {
    ...
    const resolve = (idValue: string): { id: string; kind: StructureTargetKind } | undefined => {
        const id = idValue.trim();
    ...
    const steps: StructureActionAuthoritySnapshot[] = [];
    const snapshot = (): StructureActionAuthoritySnapshot => ({
...
export function deriveFluxerBotGuildStructureCursorAuthority(input: {
    actions: readonly StructureApplyActionForValidation[];
    cursor: number;
    ...
    | { ok: true; idMap: Record<string, string>; knownTargetKinds: Record<string, StructureTargetKind> }
    | StructureActionReferenceValidationFailure {
    const graphValidation = validateFluxerBotGuildStructureActionReferences({
    ...
    const replay = simulateFluxerBotGuildStructureActionAuthority({
        actions: input.actions.slice(0, input.cursor),
        guildId: input.guildId,
        idMap: input.initialIdMap,
        knownTargetKinds: input.knownTargetKinds,
        resolveCreatedTargetId: (sourceId) => input.executionIdMap[sourceId],
        ...(input.sourceGuildId ? { sourceGuildId: input.sourceGuildId } : {}),
    });
    if (!replay.ok) {
    ...
    if (!sameStringRecord(replay.durableIdMap, input.executionIdMap)) return invalidAuthority(input.actions);
...
function validateActionReferences(
    action: StructureApplyActionForValidation,
    resolve: (id: string) => { id: string; kind: StructureTargetKind } | undefined,
    resolveRoleOverwrite: (id: string) => { id: string; kind: 'role' } | undefined
): StructureActionReferenceValidationError | undefined {
    if (action.targetType === 'channel-order') {
        ...
        const errorType = validateChannelOrderReferences(
            action.after as ApplyFluxerBotGuildStructureChannelOrderInput[],
            resolve
        );
    ...
    if (action.targetType === 'role-order') {
        ...
        const errorType = validateRoleOrderReferences(
            action.after as ApplyFluxerBotGuildStructureRoleOrderInput[],
            resolve
        );
        return errorType ? { errorType } : undefined;
    }
...
function validateChannelOrderReferences(
    order: ApplyFluxerBotGuildStructureChannelOrderInput[],
    resolve: (id: string) => { id: string; kind: StructureTargetKind } | undefined
...
function validateRoleOrderReferences(
    order: ApplyFluxerBotGuildStructureRoleOrderInput[],
    resolve: (id: string) => { id: string; kind: StructureTargetKind } | undefined
): 'structure-order-mapping-missing' | 'structure-order-plan-invalid' | undefined {
    const sourceIds = new Set<string>();
    for (const item of order) {
...
function invalidAuthority(
    ...
    return {
        ok: false,
        actionId: actions[0]?.id ?? 'execution-plan',
        errorType: 'invalid-value',
        field: 'targetId',
    };
}

function actionTargetKind(targetType: string): StructureTargetKind | undefined {
    return targetType === 'role' || targetType === 'category' || targetType === 'channel' ? targetType : undefined;
}

function isStructureTargetKind(value: unknown): value is StructureTargetKind {
...
export function normalizeRequestedChannelPositions(
    ...
    for (const [parentSourceId, channels] of [...grouped.entries()].sort(([left], [right]) =>
        ...
        const parentId = parentSourceId ? resolveStructureReference(parentSourceId, mapping) : null;
        ...
        for (const [position, channel] of [...channels]
            ...
            .entries()) {
            const channelId = resolveStructureReference(channel.sourceId.trim(), mapping);
...
export function normalizeRequestedRolePositions(
    ...
    for (const role of [...roleOrder].sort(compareRequestedRoleOrder)) {
        const targetId = resolveStructureReference(role.sourceId.trim(), mapping);
