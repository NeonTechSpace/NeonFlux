import { isProtectedBlueprintRole, type BlueprintChannel, type BlueprintRole } from './contracts.js';
import { resolveRequestedCategoryId, type CollectionIdentity } from './channel-projection.js';
import {
    BlueprintAmbiguousRoleIdentityError,
    BlueprintInvalidRoleMappingError,
    projectBlueprintRoles,
    type BlueprintRoleIdentityConflict,
} from './role-projection.js';
import type { BlueprintSnapshot } from './snapshot.js';
import type { BlueprintPlanDecision, BlueprintPolicy } from './plan.js';

export class BlueprintAmbiguousIdentityError extends Error {
    readonly code = 'ambiguous-structure-identity';
    readonly conflicts: BlueprintRoleIdentityConflict[];
    readonly decisions: BlueprintPlanDecision[];

    constructor(conflicts: BlueprintRoleIdentityConflict[] = []) {
        super(
            'Server blueprint contains an ambiguous identity match. Choose which existing object each source object maps to.'
        );
        this.name = 'BlueprintAmbiguousIdentityError';
...
export class BlueprintInvalidIdentityMappingError extends Error {
    readonly code = 'invalid-identity-mapping';

    constructor(message: string) {
        super(message);
        this.name = 'BlueprintInvalidIdentityMappingError';
    }
}
...
export function buildKnownTargetKinds(current: BlueprintSnapshot): Record<string, 'role' | 'category' | 'channel'> {
...
export function projectRoles(
    ...
    requested: BlueprintSnapshot,
    mode: BlueprintPolicy,
    retainUnmatchedCurrentRoles: boolean,
    roleMappings?: Record<string, string>
): ReturnType<typeof projectBlueprintRoles> {
    try {
        return projectBlueprintRoles({
        ...
        if (error instanceof BlueprintAmbiguousRoleIdentityError) {
            throw new BlueprintAmbiguousIdentityError(error.conflicts);
        }
        if (error instanceof BlueprintInvalidRoleMappingError) {
            throw new BlueprintInvalidIdentityMappingError(error.message);
        }
        throw error;
    }
}
...
export function buildCategoryIdentity(
    currentCategories: readonly BlueprintChannel[],
    requestedCategories: readonly BlueprintChannel[],
...
export function buildChannelIdentity(
    currentChannels: readonly BlueprintChannel[],
    requestedChannels: readonly BlueprintChannel[],
...
function buildCollectionIdentity<TItem extends { id: string }>(
    ...
    for (const [sourceId, targetId] of Object.entries(mappings ?? {})) {
        if (!requestedItems.some((item) => item.id === sourceId) || !currentById.has(targetId)) {
            throw new BlueprintInvalidIdentityMappingError(
    ...
    if ([...fallbackUseCounts.values()].some((count) => count > 1)) {
        throw new BlueprintAmbiguousIdentityError();
    }
