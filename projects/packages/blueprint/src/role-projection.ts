...
type BlueprintRoleProjectionEntry = {
    logicalId: string;
    sourceId?: string;
    targetId?: string;
    name: string;
...
export class BlueprintAmbiguousRoleIdentityError extends Error {
    ...
    constructor(conflicts: BlueprintRoleIdentityConflict[]) {
        ...
        this.name = 'BlueprintAmbiguousRoleIdentityError';
        this.conflicts = conflicts;
    }
}

export class BlueprintInvalidRoleMappingError extends Error {
    readonly code = 'invalid-role-mapping';

    constructor(message: string) {
        super(message);
        this.name = 'BlueprintInvalidRoleMappingError';
    }
}

export function projectBlueprintRoles(input: {
...
}): { identity: BlueprintRoleIdentity; projection: BlueprintRoleProjection } {
...
type CanonicalRole = BlueprintRole & { canonicalRank: number };

function canonicalizeRoles(roles: readonly BlueprintRole[]): CanonicalRole[] {
    return roles
...
function resolveRoleIdentity(input: {
    ...
    for (const [sourceId, targetId] of Object.entries(input.roleMappings ?? {})) {
        if (!requestedById.has(sourceId)) {
            throw new BlueprintInvalidRoleMappingError(
        ...
        if (usedCurrentIds.has(targetId)) {
            throw new BlueprintInvalidRoleMappingError(`Role mapping target "${targetId}" is used more than once.`);
        }
    ...
    for (const [nameKey, requestedGroup] of requestedGroups) {
        ...
        if (!assignment) {
            throw new BlueprintAmbiguousRoleIdentityError([
                {
                    targetType: 'role',
...
function isProtectedSnapshotRole(role: BlueprintRole, guildId: string | undefined): boolean {
    return (
        isProtectedBlueprintRole(role) || (typeof guildId === 'string' && role.id === guildId) || isEveryoneRole(role)
    );
}

function isEveryoneRole(role: BlueprintRole): boolean {
