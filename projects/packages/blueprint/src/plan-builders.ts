...
export function findUnsupportedChannelChanges(
    ...
    const blockers: BlueprintPlanBlocker[] = [];
...
export function buildDecisions(input: {
    ...
    const decisions: BlueprintPlanDecision[] = [];
    ...
    for (const [targetType, requestedItems, currentItems, identity] of collections) {
        for (const source of requestedItems) {
            if (targetType === 'role' && isProtectedSnapshotRole(source as BlueprintRole, input.requested.guildId)) {
                decisions.push({
                    targetType,
            ...
            if (blocker && targetId) {
                decisions.push({
                    ...
                    changes: blocker.fields.map((field) => ({
                        field,
                        before: (currentItems.find((item) => item.id === targetId) as BlueprintChannel | undefined)?.[
                            field
                        ],
                        after: (source as BlueprintChannel)[field],
                    })),
                });
                continue;
            }
            ...
            const update = input.changes.find(
                (change) =>
                    change.actionType === 'update' && change.targetType === targetType && change.targetId === targetId
            );
            const decision: BlueprintPlanDecision = {
                targetType,
            ...
            if (update && 'changes' in update.details && Array.isArray(update.details.changes)) {
                decision.changes = update.details.changes as NonNullable<BlueprintPlanDecision['changes']>;
            }
        ...
        for (const target of currentItems) {
            ...
            const protectedTarget =
                targetType === 'role' && isProtectedSnapshotRole(target as BlueprintRole, input.current.guildId);
            decisions.push({
                targetType,
...
function sortDecisions(decisions: BlueprintPlanDecision[]): BlueprintPlanDecision[] {
...
export function buildProjectedSnapshot(input: {
    ...
    const roles = input.roleProjection.roles.flatMap((entry): BlueprintRole[] => {
    ...
    const mapCollection = (
        requestedItems: readonly BlueprintChannel[],
        currentItems: readonly BlueprintChannel[],
        currentById: ReadonlyMap<string, BlueprintChannel>,
...
function canonicalizeProjectedChannel(channel: BlueprintChannel): BlueprintChannel {
    return {
        ...channel,
...
function compareProjectedChannels(left: BlueprintChannel, right: BlueprintChannel): number {
    return (
