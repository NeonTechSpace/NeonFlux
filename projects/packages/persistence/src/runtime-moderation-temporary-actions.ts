...
import {
    cancelPendingModerationTemporaryActionsByTarget as cancelPendingModerationTemporaryActionsByTargetPostgres,
    createModerationTemporaryAction as createModerationTemporaryActionPostgres,
    findPendingModerationTemporaryActionByTarget as findPendingModerationTemporaryActionByTargetPostgres,
    listDueModerationTemporaryActions as listDueModerationTemporaryActionsPostgres,
    updateModerationTemporaryActionStatus as updateModerationTemporaryActionStatusPostgres,
    type GuildFeatureRepositoryError,
    type ModerationTemporaryActionRecord,
    type ModerationTemporaryActionRepositoryError,
    type ModerationTemporaryActionStatus,
} from '@neonflux/db';
...
type PostgresTemporaryActionDb = Parameters<typeof createModerationTemporaryActionPostgres>[0];
type TemporaryActionDb = ConvexPersistenceDatabase | PostgresTemporaryActionDb;
...
export async function createModerationTemporaryAction(
    ...
    if (!isConvexPersistenceDatabase(db)) return createModerationTemporaryActionPostgres(db, input);
    ...
    try {
        ...
        )) as ConvexModerationTemporaryActionRecord;
...
export async function findPendingModerationTemporaryActionByTarget(
    ...
    if (!isConvexPersistenceDatabase(db)) return findPendingModerationTemporaryActionByTargetPostgres(db, input);
    ...
    try {
        ...
        )) as ConvexModerationTemporaryActionRecord | null;
...
export async function listDueModerationTemporaryActions(
    ...
    if (!isConvexPersistenceDatabase(db)) return listDueModerationTemporaryActionsPostgres(db, input);
...
export async function cancelPendingModerationTemporaryActionsByTarget(
    ...
    if (!isConvexPersistenceDatabase(db)) return cancelPendingModerationTemporaryActionsByTargetPostgres(db, input);
...
export async function updateModerationTemporaryActionStatus(
    ...
    if (!isConvexPersistenceDatabase(db)) return updateModerationTemporaryActionStatusPostgres(db, input);
