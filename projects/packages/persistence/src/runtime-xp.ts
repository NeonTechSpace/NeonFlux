...
import {
    addGuildUserXp as addGuildUserXpPostgres,
    findGuildUserXp as findGuildUserXpPostgres,
    findGuildUserXpRank as findGuildUserXpRankPostgres,
    findXpSettingsByGuildId as findXpSettingsByGuildIdPostgres,
    grantGuildUserXp as grantGuildUserXpPostgres,
    listGuildXpLeaderboard as listGuildXpLeaderboardPostgres,
    upsertXpRoleReward as upsertXpRoleRewardPostgres,
    upsertXpSettings as upsertXpSettingsPostgres,
    type GrantGuildUserXpResult,
    type GuildFeatureRepositoryError,
    type GuildUserXpRank,
    type GuildUserXpRecord,
    type XpGrantRecord,
    type XpGrantSource,
    type XpRepositoryError,
    type XpRoleRewardRecord,
...
type PostgresXpDb = Parameters<typeof upsertXpSettingsPostgres>[0];
type XpDb = ConvexPersistenceDatabase | PostgresXpDb;
...
export async function upsertXpSettings(
    ...
    if (!isConvexPersistenceDatabase(db)) {
        return upsertXpSettingsPostgres(db, input);
    }
...
export async function findXpSettingsByGuildId(
    ...
    if (!isConvexPersistenceDatabase(db)) {
        return findXpSettingsByGuildIdPostgres(db, input);
    }
...
export async function addGuildUserXp(
    ...
    if (!isConvexPersistenceDatabase(db)) {
        return addGuildUserXpPostgres(db, input);
    }
...
export async function grantGuildUserXp(
    ...
    if (!isConvexPersistenceDatabase(db)) {
        return grantGuildUserXpPostgres(db, input);
    }
...
export async function upsertXpRoleReward(
    ...
    if (!isConvexPersistenceDatabase(db)) {
        return upsertXpRoleRewardPostgres(db, input);
    }
...
export async function findGuildUserXp(
    ...
    if (!isConvexPersistenceDatabase(db)) {
        return findGuildUserXpPostgres(db, input);
    }
...
export async function findGuildUserXpRank(
    ...
    if (!isConvexPersistenceDatabase(db)) {
        return findGuildUserXpRankPostgres(db, input);
    }
...
export async function listGuildXpLeaderboard(
    ...
    if (!isConvexPersistenceDatabase(db)) {
        return listGuildXpLeaderboardPostgres(db, input);
    }
