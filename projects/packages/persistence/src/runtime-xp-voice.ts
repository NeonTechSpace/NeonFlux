...
import {
    closeXpVoiceSession as closeXpVoiceSessionPostgres,
    startXpVoiceSession as startXpVoiceSessionPostgres,
    transitionXpVoiceSession as transitionXpVoiceSessionPostgres,
    type ClosedXpVoiceSession,
    type GuildFeatureRepositoryError,
    type XpVoiceSessionRecord,
    type XpVoiceSessionRepositoryError,
    type XpVoiceSessionTransition,
} from '@neonflux/db';
...
type PostgresXpVoiceDb = Parameters<typeof transitionXpVoiceSessionPostgres>[0];
type XpVoiceDb = ConvexPersistenceDatabase | PostgresXpVoiceDb;
...
export async function transitionXpVoiceSession(
    ...
    if (!isConvexPersistenceDatabase(db)) {
        return transitionXpVoiceSessionPostgres(db, input);
    }
...
export async function startXpVoiceSession(
    ...
    if (!isConvexPersistenceDatabase(db)) {
        return startXpVoiceSessionPostgres(db, input);
    }
...
export async function closeXpVoiceSession(
    ...
    if (!isConvexPersistenceDatabase(db)) {
        return closeXpVoiceSessionPostgres(db, input);
    }
